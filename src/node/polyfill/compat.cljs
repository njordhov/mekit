...
(let [xhr-module "xmlhttprequest"]
  (when-let [xhr (nodejs/require xhr-module)]
    (set! js/XMLHttpRequest
      (case xhr-module
        "xhr2" xhr
        "xmlhttprequest" (set! js/XMLHttpRequest (.-XMLHttpRequest xhr))))))

;; needed for isomorphic material-ui. May cause problems.
